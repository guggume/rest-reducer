import refreshToken, {
  canRefreshToken,
  isRefreshing,
  addToQueue
} from "./refresh";
import { getParams, dispatchSuccess, dispatchErrors } from "../utils";

export async function handleResponse(response, options, replay = false) {
  const { ACTIONS, dispatch, id, patchId, config, getState } = options;

  if (!response.status) {
    dispatchErrors(dispatch, null, ACTIONS.FAILURE, { id, patchId });
  } else if (response.status >= 200 && response.status < 300) {
    const data = await response.json();

    dispatchSuccess(
      dispatch,
      response.status,
      data.value,
      data.permissions || {},
      ACTIONS.SUCCESS,
      { id, patchId }
    );
  } else if (
    response.status === 401 &&
    canRefreshToken(config, getState()) &&
    !replay
  ) {
    refreshToken(options, "patch", response);
  } else if (response.status >= 400 && response.status < 500) {
    const data = await response.json();

    dispatchErrors(dispatch, response, ACTIONS.FAILURE, {
      id,
      patchId,
      error: data.error
    });
  } else {
    dispatchErrors(dispatch, response, ACTIONS.FAILURE, { id, patchId });
  }
}

const CONTENT_TYPE = "application/JSON";

export default async function(options, replay = false) {
  const {
    ACTIONS,
    dispatch,
    getState,
    globalParams,
    url,
    patchId,
    id,
    value,
    meta,
    params,
    raw,
    config
  } = options;

  if (!replay) {
    dispatch({
      type: ACTIONS.PENDING,
      payload: { id, patchId, value, meta, code: -1 }
    });
  }

  if (isRefreshing()) {
    addToQueue(options, "patch");
  } else {
    try {
      const response = await fetch(
        url(id, value, meta),
        getParams(
          getState(),
          globalParams,
          {
            method: "PATCH",
            headers: {
              "Content-Type": CONTENT_TYPE
            },
            body: raw ? value : JSON.stringify(value),
            ...params
          },
          config
        )
      );
      handleResponse(response, options);
    } catch (e) {
      handleResponse({}, options);
    }
  }
}
